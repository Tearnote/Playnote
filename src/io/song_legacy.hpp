...
public:
	...
	explicit SongLegacy(fs::path domain);
...
inline auto SongLegacy::FileRef::load() -> vector<byte>
{
...
inline SongLegacy::SongLegacy(fs::path domain):
	domain{move(domain)}
{
...
inline auto SongLegacy::load_bms(string_view filename) const -> vector<byte>
{
	if (type == DomainType::Archive) {
...
inline auto SongLegacy::remove_prefix(string_view str) const -> string_view
{
...
template<callable<void(SongLegacy::FileRef)> Func>
void SongLegacy::for_each_file(Func&& func)
{
	if (type == DomainType::Archive) {
		auto archive_file = read_file(domain);
		auto archive = lib::archive::open_read(archive_file.contents);
